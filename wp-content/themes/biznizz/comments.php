...
<?php if ( have_comments() ) : ?>
	...
	<?php if ( ! empty($comments_by_type['comment']) ) : ?>
		...
		<h3 class="cufon"><?php comments_number(__('No Responses', 'woothemes'), __('One Response', 'woothemes'), __('% Responses', 'woothemes') );?> <?php _e('to', 'woothemes') ?> &#8220;<?php the_title(); ?>&#8221;</h3>
...
<h3 class="cufon"><?php comment_form_title( __('Leave a Reply', 'woothemes'), __('Leave a Reply to %s', 'woothemes') ); ?></h3>
	...
	<?php else : //No registration required ?>
		...
		<?php else : //If user is not logged in ?>
			...
			<p>
				...
				<label for="author"><?php _e('Name', 'woothemes') ?> <?php if ($req) ?> (<?php _e('Required', 'woothemes'); ?>) <?php ; ?></label>
			</p>
